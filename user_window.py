...
class UserWindow(ctk.CTkToplevel):
    ...
    def open_realtime_use(self, options: str):
        if options == "Use Model 1":
            mu1.RunModelClass1().deiconify()
        elif options == "Use Model 2":
    ...
    def __init__(self, user_data):
        ...
        open_webcam = ctk.CTkOptionMenu(menubar_frame, values=["Use Model 1", "Use Model 2"],
                                        fg_color="#fd8891",
                                        button_color="#8A3A75", button_hover_color="#AA5A95",
                                        command=self.open_realtime_use
                                        )
        open_webcam.grid(row=0, column=1, pady=20, padx=26)
        open_webcam.set("Open Web Cam")
