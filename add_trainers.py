...
class AddTrainer(ctk.CTkToplevel):
    ...
    def __init__(self):
        ...
        def admin_registration_activity():
            ...
            if (trainer_id == '' or
                    ...
                    email == "" or contact == "" or gender == "" or occupation == "" or state == ""
                    ):
                ...
                try:
                    ...
                    exist_query = "SELECT * FROM trainer_table WHERE trainer_id = %s"
                    my_cursor.execute(exist_query, (trainer_id,))
                    exist_result = my_cursor.fetchone()
                    if exist_result:
                        showinfo("trainer already exists",
                                 "trainer already registered try different trainer ID")
                    else:
                        insert_cmd = "insert into trainer_table(trainer_id,f_name,l_name,email,contact,dob,gender,occupation,city,state,password,secrete_key) value(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                        val = [
                            (
                                trainer_id, first_name, last_name, email, contact, dob, gender, occupation, city, state,
                                password, secretkey)
                        ]
                        my_cursor.executemany(insert_cmd, val)
                        mydb.commit()
                        print(f"{my_cursor.rowcount} record inserted")
                        showinfo("Trainer is Added", "Trainer added successfully.....! ")
                        showinfo("SECRETE KEY", f"{first_name}'s secrete key is : {secretkey}")
        ...
        self.img = Image.open("image_folder/6453214.jpg")
        ...
        head_line = ctk.CTkLabel(self.frame, text="TRAINER REGISTRATION", text_color="#586998",
                                 font=("Copperplate Gothic Bold", 20, "bold"))
        head_line.place(x=18, y=20)
        field_frame = ctk.CTkFrame(master=self.frame, fg_color="#a3c9fe", corner_radius=20, width=510, height=540,
                                   border_color="#596b9b",border_width=1)
        field_frame.place(x=380, y=20)

        # admin id entry's--------------------------------------
        trainer_id_label = ctk.CTkLabel(master=field_frame,
                                      text_color="#223a79",
                                      text="Trainer ID",
                                      font=("Rockwell", 16, "bold"))
        trainer_id_label.place(x=30, y=30)

        trainer_id_entry = ctk.CTkEntry(master=field_frame,
                                        fg_color="#ddecff", border_width=0, text_color="#596b9b",
                                        font=("Rockwell", 14, "normal"), width=250)
        trainer_id_entry.place(x=130, y=30)
        # name entry--------------------------------------------
        ...
        occupation_entry = ctk.CTkOptionMenu(master=field_frame,
                                             ...
                                             button_color="#ddecff")
        occupation_entry.place(x=140, y=270)
        ...
        #                            text="This trainer is trusted user ",
        ...
        # buttons--------------------------------
        self.r_img = Image.open("image_folder/icon/plus.png")
        self.reg_image = ctk.CTkImage(light_image=self.r_img, size=(20, 20))
        add_btn = ctk.CTkButton(master=field_frame, text="Add", text_color="#fff", fg_color="#223a79",
                                     height=35,
                                     corner_radius=30, hover_color="#596b9b", image=self.reg_image,
                                     command=admin_registration_activity)
        add_btn.place(x=30, y=430)
        ...
        cancel_btn = ctk.CTkButton(master=field_frame, text="Cancel", text_color="#fff", fg_color="#223a79", height=35,
                                   corner_radius=30, hover_color="#596b9b", image=self.cancel_image,
                                   command=cancel_operation)
        self.protocol("WM_DELETE_WINDOW", cancel_operation)
        cancel_btn.place(x=250, y=430)
